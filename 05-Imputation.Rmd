...
# remove p95 as we'll re calculate here for consistency
dat_grm %<>% select(-p95)
...
a) POLYTYPE == 'FOR'
a) polygon >= 50% forested landcover
b) p95 (95th percentile of LiDAR height returns) >= 5 meters (definition of 'forest')
...
The goal of this imputation procedure is to estimate age and species composition in newly segmented forest polygons. But it is also important to assess the performance of the algorithm. To do this, we can conduct the imputation over the FRI dataset ONLY. For each FRI polygon, we find the k-nearest neighbor matches, and calculate the age and species composition to impute. We can then compare the observed age and species composition of the polygon to the imputed values. Note we have to do this calculation on the FRI dataset alone because we do not have observed age and species composition values for the newly segmented polygons. Also note that ALL attributes are imputed from the same k-nearest neighbors. The algorithm is not run for individual attributes.

We present the performance of numeric attributes (age as well as the variables used to conduct the imputation algorithm) as Root Mean Squared Difference (RMSD). RMSD is the same formula as Root Mean Squared Error but we use the term "difference" to note that we cannot impute the exact value (since the value comes from the k-nearest neighbors). We report both Raw RMSD (in the unit of measurement of each variable) as well as Scaled RMSD (Raw RMSD divided by the standard deviation of the observed data), which allows direct comparison between numeric attributes.

For species composition, we report the percent of observed and imputed values that match. Species composition is broken down into several distinct attributes:
...
dat_fri_scr %<>% select(POLYID, AGE, SPCOMP, WG, 
                        p95, rumple, zpcum8,
                        depth_q25, x, y,
                        red_edge_2)
...
The last step is to run the imputation between the screened FRI data frame and the newly segmented polygons. For each newly segmented polygon, the best k-nearest neighbors in the FRI data frame are found and used to impute age and species composition. Note we do not conduct imputation on all the newly segmented polygons, but only the polygons that are dominated by forested landcover and have p95 >= 5 meters (same criteria as FRI data screening conducted above in step 2).

Although we cannot assess performance of age and species composition when imputing into the newly segmented polygons, we can still assess the fit of the variables used in the imputation: p95, canopy cover, and Sentinel red_edge_2.

We can also review maps of the FRI age/species comp against maps of the newly segmented polygons.
...
dat_grm_imp <- dat_grm_scr %>% select(id, p95, rumple,
                                      zpcum8, depth_q25,
                                      x, y, red_edge_2) %>% na.omit
dat_fri_imp <- dat_fri_scr %>% select(p95, rumple,
                                      zpcum8, depth_q25,
                                      x, y, red_edge_2) %>% na.omit
...
# run nearest neighbor imputation k = 10
nn <- nn2(dat_fri_scaled, dat_grm_scaled, k = 10)
...
perf <- tibble(p95_raw_rmsd = rmse(dat_grm_imp$p95, dat_grm_imp$p95_imp),
               p95_scaled_rmsd = rmse(dat_grm_imp$p95, dat_grm_imp$p95_imp) /
                 sd(dat_grm_imp$p95),
               rumple_raw_rmsd = rmse(dat_grm_imp$rumple, dat_grm_imp$rumple_imp),
               rumple_scaled_rmsd = rmse(dat_grm_imp$rumple, dat_grm_imp$rumple_imp) /
                 sd(dat_grm_imp$rumple),
               zpcum8_raw_rmsd = rmse(dat_grm_imp$zpcum8, dat_grm_imp$zpcum8_imp),
               zpcum8_scaled_rmsd = rmse(dat_grm_imp$zpcum8, dat_grm_imp$zpcum8_imp) /
                 sd(dat_grm_imp$zpcum8),
               depth_q25_raw_rmsd = rmse(dat_grm_imp$depth_q25, dat_grm_imp$depth_q25_imp),
               depth_q25_scaled_rmsd = rmse(dat_grm_imp$depth_q25, dat_grm_imp$depth_q25_imp) /
                 sd(dat_grm_imp$depth_q25),
               x_raw_rmsd = rmse(dat_grm_imp$x, dat_grm_imp$x_imp),
               x_scaled_rmsd = rmse(dat_grm_imp$x, dat_grm_imp$x_imp) /
                 sd(dat_grm_imp$x),
               y_raw_rmsd = rmse(dat_grm_imp$y, dat_grm_imp$y_imp),
               y_scaled_rmsd = rmse(dat_grm_imp$y, dat_grm_imp$y_imp) /
                 sd(dat_grm_imp$y),
               red_edge_2_raw_rmsd = rmse(dat_grm_imp$red_edge_2, dat_grm_imp$red_edge_2_imp),
               red_edge_2_scaled_rmsd = rmse(dat_grm_imp$red_edge_2, dat_grm_imp$red_edge_2_imp) /
                 sd(dat_grm_imp$red_edge_2)) %>% melt
...
poly_fri$AGE[!(poly_fri$POLYID %in% dat_fri_scr$POLYID)] <- NA
...
plot(poly_grm, 'age', col = viridis(14), type = 'interval', breaks = seq(0, 140, 10),
     plg = list(x = 'topright', cex = 4, title = 'Age'),
     main = '')
title(main = 'Imputed Age of GRM Segmented Forest Stands', cex.main = 4)

plot(poly_fri, 'AGE', col = viridis(14), type = 'interval', breaks = seq(0, 140, 10),
     plg = list(x = 'topright', cex = 4, title = 'Age'),
     main = '')
title(main = 'Age of FRI Forest Stands', cex.main = 4)
...
plot(poly_grm, 'class5', col = viridis(5), type = 'classes',
     plg = list(x = 'topright', cex = 4, title = 'Species Class'),
     main = '')
title(main = 'Imputed Species Class (5) of GRM Segmented Forest Stands', cex.main = 4)

plot(poly_fri, 'class5', col = viridis(5), type = 'classes',
     plg = list(x = 'topright', cex = 4, title = 'Species Class'),
     main = '')
title(main = 'Species Class (5) FRI Forest Stands', cex.main = 4)
```
...
plot(poly_grm, 'class3', col = viridis(3), type = 'classes',
     plg = list(x = 'topright', cex = 4, title = 'Species Class'),
     main = '')
title(main = 'Imputed Species Class (3) of GRM Segmented Forest Stands', cex.main = 4)

plot(poly_fri, 'class3', col = viridis(3), type = 'classes',
     plg = list(x = 'topright', cex = 4, title = 'Species Class'),
     main = '')
title(main = 'Species Class (3) FRI Forest Stands', cex.main = 4)
```
...
ggplot(as.data.frame(poly_fri), aes(x = AGE)) +
  geom_density(fill = 'grey') +
  geom_vline(aes(xintercept = median(AGE, na.rm = T)),
             linetype = "dashed",
             size = 0.6) +
...
ggplot(dat_grm_c3, aes(x = "", y = prop, fill = class3)) +
  ...
  theme_void() +
  geom_text(aes(y = ypos, label = str_c(lbl, "%")), size = 20) +
  theme(legend.title = element_text(size = 60),
        legend.text = element_text(size = 50),
        legend.key.width = unit(2, 'cm'),
        plot.title = element_text(size=60)) +
  labs(fill = "3 Species Classification") +
  ggtitle("Imputed 3 Species Classification of GRM Segmented Forest Stands")
...
ggplot(dat_fri_c3, aes(x = "", y = prop, fill = class3)) +
  ...
  theme(legend.title = element_text(size = 60),
        ...
        plot.title = element_text(size=60)) +
  labs(fill = "3 Species Classification") +
  ggtitle("3 Species Classification of FRI Forest Stands")

```

```{r, imputationp4g, results=FALSE, message=FALSE, warning=FALSE, fig.width=18, fig.height=12}
...
ggplot(dat_grm_c5, aes(x = "", y = prop, fill = class5)) +
  ...
  ggtitle("Imputed 5 Species Classification of GRM Segmented Forest Stands")
...
ggplot(dat_fri_c5, aes(x = "", y = prop, fill = class5)) +
  ...
  theme(legend.title = element_text(size = 60),
        ...
        plot.title = element_text(size=60)) +
  labs(fill = "5 Species Classification") +
  ggtitle("5 Species Classification of FRI Forest Stands")
