...
for(m in methods){
  ...
  for(i in 1:10){
    ...
    if(m == 'randomForest') {
      imp <- yai(x = x, y = y, method = 'randomForest', ntree = 100*NCOL(y))
    }else imp <- yai(x = x, y = y, method = m)
  ...
  # add to final df
  if(which(methods == m) == 1){
    # if first method create df
    rmsd_df <- cbind(data.frame(method = m),
                    data.frame(colMeans(perform_df)) %>% t)
...
# write as csv
# write.csv(rmsd_df, file = 'D:/ontario_inventory/imputation/ten_fold_validation/rmsd_10_fold_methods_ht.csv')
