...
# This version of the code runs a pca to lower the number of input variables

# And only uses numeric target variables that match our input variables: HT, BA, CC
...
# try to improve ref variables by running a pca
# first subset all possible ALS vars
ref_vars <-
  c('cc', 'p95', 'cv', 'kur', 'ske')
dat_pca <- dat[, ref_vars]

# check correlation between vars
cor_matrix <- cor(dat_pca)
diag(cor_matrix) <- 0
library(corrplot)
...
# investigate proportion of var explained
fviz_eig(pca)
